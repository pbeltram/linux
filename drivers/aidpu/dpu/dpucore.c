...
struct device *dev_handler;
static struct dpu_dev dpudev;

dpu_caps_t dpu_caps;

// debug vars
int _run_counter[MAX_CORE_NUM] = { 0 };
int _int_counter[MAX_CORE_NUM] = { 0 };
...
unsigned long dpu_alloc_mem(uint32_t memsize)
{
  ...
  if (NULL != virtaddr) {
    ...
    if (pnewnode) {
      ...
      phy_addr = 0;
      dev_err(dev_handler, "kmalloc fail when adding memory node.\n");
    }
    return phy_addr;
  }
  else {
...
int dpu_free_mem(void *paddr)
{
  ...
  dev_err(dev_handler, "kfree memory failed, address=0x%p.\n", paddr);
...
inline void _show_debug_info(void)
{
  ...
  dev_info(dev_handler, "[DPU debug info]\nlevel = %d\n", debuglevel);
  for (i = 0; i < DPU_CORE_NUM; i++) {
    dev_info(dev_handler, "Core %d schedule  counter: %d\n", i, _run_counter[i]);
    dev_info(dev_handler, "Core %d interrupt counter: %d\n", i, _int_counter[i]);
  }
}
/*----------------------------------------------------------------------------*/
...
inline void _show_dpu_regs(void)
{
  ...
  for (i = 0; i < DPU_CORE_NUM; i++) {
    ...
    for (j = 0; j < 8; j++) {
      dev_info(dev_handler, "%-8s%d\t: 0x%.8x\n", "COM_ADDR_L", j, pdpureg->ctlreg[i].com_addr[j * 2]);
      dev_info(dev_handler, "%-8s%d\t: 0x%.8x\n", "COM_ADDR_H", j, pdpureg->ctlreg[i].com_addr[j * 2 + 1]);
    }
    dev_info(dev_handler, "\n");
  }
...
int dpu_run(struct ioc_kernel_run2_t *prun)
{
  ...
  if (dpu_core == DPU_CORE_NUM) {
    ...
    dev_err(dev_handler, "ERR_CORE_NUMBER!\n");
    spin_lock_irqsave(&corelock, flags);
    for (i = 0; i < DPU_CORE_NUM; i++) {
      dev_info(dev_handler, "Core%d state:%d,task_id:%ld\n",
          i, dpudev.state[i], dpudev.task_id[i]);
    }
    spin_unlock_irqrestore(&corelock, flags);
    return -EINTR;
  }
  ...
  if (ret == 0) {
    dev_err(dev_handler, "[taskID %ld]Core %d Run timeout,failed to get finish interrupt!\n",
        prun->handle_id, dpu_core);
    _show_debug_info();
    ...
    __PDEBUG(PLEVEL_INFO, "program interrupted by user!\n");
    __PDEBUG(PLEVEL_INFO, "[ret=%d][core=%d,status=%d]\n",
        ret, dpu_core, dpudev.state[dpu_core]);

    _dpu_regs_init(dpu_core);
  }
  else {
    __PDEBUG(PLEVEL_DBG, "[core %i]Task takes: %lld ns\n",
        dpu_core, dpudev.time_end[dpu_core] - dpudev.time_start[dpu_core]);
...
static long dpu_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
  ...
  __PDEBUG(PLEVEL_DBG, "name:%s,func:%s\n", current->comm, __func__);
  __PDEBUG(PLEVEL_INFO, "IOCTL CMD = 0x%x (1:Create;2:Alloc;3:Free;4:Run;6:Done)\n", cmd);
  ...
  switch (cmd) {
    ...
    default: {
      dev_err(dev_handler, "IOCTL CMD=%d NOT SUPPORTED!\n", cmd);
      ret = -EPERM;
...
static ssize_t proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *f_pos)
{
  ...
  __PDEBUG(PLEVEL_DBG, "Debug level set to %d.", debuglevel);
...
struct device_node *dpu_compatible_node(const char *compat)
{
  ...
  for (idx=0; idx<max; idx++) {
    ...
    if (pdpu_node) {
      dev_info(dev_handler, "Found compatible node '%s'.\n", dst_node);
      break;
    }
  }

  if (pdpu_node == NULL)
    dev_info(dev_handler, "Compatible node for '%s' not found.\n", compat);
...
static int dpu_probe(struct platform_device *pdev)
{
  int ret, i;
  void *prop;
//  struct device_node *dpucore_node;
  ...
  dev_info(dev_handler, "Probing ...\n");

//  dpucore_node = dpu_compatible_node("dpucore");

  pdpu_node = dpu_compatible_node("dpu");
  if (!pdpu_node) {
    dev_err(dev_handler, "Not found DPU device node!\n");
    return -ENXIO;
  }
  else {
    ...
    if (base_addr_dtsi) {
      dev_info(dev_handler, "Found DPU signature addr = 0x%lx in device-tree\n", base_addr_dtsi);
      signature_addr = base_addr_dtsi + 0x00F00000;
    }

    if (signature_addr != SIG_BASE_NULL) {
      dev_info(dev_handler, "Checking DPU signature at addr = 0x%lx.\n", signature_addr);
      signature_va = ioremap((phys_addr_t)signature_addr, 1 * sizeof(signature_field));
    ...
    if ((signature_field & SIG_MAGIC_MASK) == SIG_MAGIC) {
      ...
      signature_va = ioremap((phys_addr_t)signature_addr, signature_length * sizeof(signature_field));
      ...
      if (0x0 == dpu_caps.magic) {
        ...
        for (i = 0; i < VER_MAX_ENTRY; i++) {
          ...
          if (signature_temp != 0) {
            dev_err(dev_handler, "Unknown reserved field found in DPU signature at offset: %#X.\n", i * 4);
            dev_warn(dev_handler, "Try to update DPU driver to the latest version to resolve this issue.\n");
            return -ENXIO;
          }
        }
      }
      ...
      if (dpu_caps.hp_width >= DPU_HP_WIDTH_RESERVE) {
        dev_err(dev_handler, "Invalid hp width '%d' found in DPU signature.\n",
           dpu_caps.hp_width);
        return -ENXIO;
      }

      if (dpu_caps.data_width >= DPU_DATA_WIDTH_RESERVE) {
        dev_err(dev_handler, "Invalid data width '%d' found in DPU signature.\n",
           dpu_caps.data_width);
        return -ENXIO;
      }

      if (dpu_caps.bank_group >= DPU_BANK_GROUP_RESERVE ||  DPU_BANK_GROUP_1 == dpu_caps.bank_group) {
        dev_err(dev_handler, "Invalid bank group '%d' found in DPU signature.\n",
           dpu_caps.bank_group);
        return -ENXIO;
      }
      ...
      for (i = 0; i < 8; i++) {
        ...
        irqs[i] = signature_temp & 0x8 ?
            (signature_temp & 0x7) + dpu_caps.irq_base1 : (signature_temp & 0x7) + dpu_caps.irq_base0;
      }
      ...
      if (dpu_caps.dpu_cnt == 0) {
        dev_err(dev_handler, "No DPU core found in current configuration of DPU IP!\n");
        return -ENXIO;
      }
      ...
      dpu_caps.conv_depthwise.version = field_mask_value(signature_field, CONV_DEPTHWISE_MASK);
      dpu_caps.relu_leaky.version = field_mask_value(signature_field, RELU_LEAKY_MASK);
      dpu_caps.relu_p.version = field_mask_value(signature_field, RELU_P_MASK);

      // offset 7
      signature_field = ioread32(signature_va + 7);
      dpu_caps.serdes_nonlinear.version = field_mask_value(signature_field, SERDES_NONLINEAR_MASK);
      ...
      dpu_caps.bt1120.version = field_mask_value(signature_field, BT1120_VER_MASK);
      ...
      dpu_caps.fullconnect.enable = field_mask_value(extension, DPU_EXT_FULLCONNECT);
      dpu_caps.fullconnect.version = field_mask_value(signature_field, FC_VER_MASK);
      ...
      dpu_caps.fullconnect.irq =  signature_temp & 0x8 ?
          (signature_temp & 0x7) + dpu_caps.irq_base1 : (signature_temp & 0x7) + dpu_caps.irq_base0;

      dpu_caps.softmax.enable = field_mask_value(extension, DPU_EXT_SOFTMAX);
      dpu_caps.softmax.version = field_mask_value(signature_field, SOFTMAX_VER_MASK);
      dpu_caps.softmax.valid = field_mask_value(signature_field, SOFTMAX_VLD_MASK);
      ...
      dpu_caps.resize.version = field_mask_value(signature_field, RESIZE_VER_MASK);
      ...
      if (0x0 == dpu_caps.magic) {
        dpu_caps.p_dpu_info = kzalloc(sizeof(dpu_info_t) * dpu_caps.dpu_cnt, GFP_ATOMIC);
        if (!dpu_caps.p_dpu_info) {
          dev_err(dev_handler, "kzalloc failed.\n");
          return -ENXIO;
        }
        else {
          int idx;
          for (idx = 0; idx < dpu_caps.dpu_cnt; idx++) {
            (*(dpu_caps.p_dpu_info + idx)).base.dpu_arch = dpu_info.base.dpu_arch;
            (*(dpu_caps.p_dpu_info + idx)).base.dpu_freq = dpu_info.base.dpu_freq;
            (*(dpu_caps.p_dpu_info + idx)).dpu_target = dpu_info.dpu_target;
            (*(dpu_caps.p_dpu_info + idx)).irq = irqs[idx];

            if ((*(dpu_caps.p_dpu_info + idx)).base.dpu_arch > DPU_ARCH_RESERVE) {
              dev_err(dev_handler, "Unknown DPU arch type '%d' found in DPU signature.\n",
                (*(dpu_caps.p_dpu_info + idx)).base.dpu_arch);

              dev_warn(dev_handler, "Try to update DPU driver to the latest version to resolve this issue.\n");
              return -ENXIO;
            }
            if ((*(dpu_caps.p_dpu_info + idx)).dpu_target > DPU_TARGET_RESERVE) {
              dev_err(dev_handler, "Unknown DPU target type '%d' found in DPU signature.\n",
      ...
      dev_info(dev_handler, "DPU signature checking done!\n");
      ...
      dev_err(dev_handler, "Invalid 'signature-addr' value specified in DPU device tree, please check.\n");
      return -ENXIO;
    }
    else {
      dev_warn(dev_handler, "DPU signature NOT found, fallback to device-tree.\n");
      ...
      if (dpu_caps.dpu_cnt > 0) {
        ...
        if (!dpu_caps.p_dpu_info) {
          ...
          int idx;
          for (idx = 0; idx < dpu_caps.dpu_cnt; idx++) {
            //default to target:1.1.3 due to massively deployed
            (*(dpu_caps.p_dpu_info + idx)).dpu_target = DPU_TARGET_V1_1_3;
          }
        }
      }
      else {
  ...
  if ((DPU_CORE_NUM == 0) || (DPU_CORE_NUM > MAX_CORE_NUM)) {
    dev_err(dev_handler, "Core number %d invalid!\n", DPU_CORE_NUM);
    return -EINVAL;
  }
  ...
  if (res) {
    ...
    if (!pdpureg) {
      dev_err(dev_handler, "Map DPU registers error.\n");
      return -ENXIO;
    }
    ...
    dev_warn(dev_handler, "Not found DPU reg resources!\n");
  }
...
//    dpudev.irq_no[i] = dpucore_node ? irq_of_parse_and_map(dpucore_node, i) : platform_get_irq(pdev, i);
    dpudev.irq_no[i] = pdpu_node ? irq_of_parse_and_map(pdpu_node, i) : platform_get_irq(pdev, i);
    if (dpudev.irq_no[i] < 0) {
      dev_err(dev_handler, "IRQ resource not found for DPU core %d.\n", i);
      return dpudev.irq_no[i];
    }

    ret = request_irq(dpudev.irq_no[i], (irq_handler_t)dpu_isr, 0, "dpu_isr", NULL);
    if (ret != 0) {
      dev_err(dev_handler, "Request IRQ %d failed!\n", dpudev.irq_no[i]);
      return ret;
    }
    else {
      dev_info(dev_handler, "IRQ %d handler configured.\n", dpudev.irq_no[i]);
    }
  }
  ...
  ret = misc_register(&dpudev.dev);
  if (ret != 0) {
    dev_err(dev_handler, "Device resgister failed.\n");
    return ret;
  }

  dev_info(dev_handler, "Probe OK.\n");
