...
//		period_cycles = mul_u64_u64_div_u64(period_cycles, rate, NSEC_PER_SEC);
    period_cycles = mul_u64_u64_shr(period_cycles, rate, 0);
    period_cycles = div_u64(period_cycles, NSEC_PER_SEC);
...
//		return dev_err_probe(dev, ret, "could not read #pwm-cells\n");
    dev_err(dev, "error %pe: could not read #pwm-cells.\n", ERR_PTR(ret));
	  return ERR_PTR(ret);
	}
...
//	priv->base = devm_platform_ioremap_resource(pdev, 0);
  ...
  priv->base = devm_ioremap_resource(&pdev->dev, res);
	...
	if (IS_ERR(clk_cs)) {
//		return dev_err_probe(dev, PTR_ERR(clk_cs), "ERROR: timer input clock not found\n");
    dev_err(dev, "error %pe: ERROR: timer input clock not found.\n", PTR_ERR(clk_cs));
...
//		return dev_err_probe(dev, ret, "Clock enable failed\n");
    dev_err(dev,"error %pe: Clock enable failed\n", ERR_PTR(ret));
    return ERR_PTR(ret);
	}
...
//		clk_rate_exclusive_put(priv->clk);
    ...
    dev_err(dev, "error %pe: ERROR: Could not register PWM chip.\n", ERR_PTR(ret));
    return ERR_PTR(ret);
	}
