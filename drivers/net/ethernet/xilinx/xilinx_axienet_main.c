...
//TODO: Check after merge v5.15_LTS
//#ifdef CONFIG_XILINX_TSN_PTP/
//		if (lp->is_tsn) {
					      ...
					      struct net_device *ndev)
{
	...
	u32 dma_mask = XAE_DMA_MASK_MIN;
  ret = of_property_read_u32(np, "xlnx,addrwidth", &dma_mask);
  if (ret <  0 || dma_mask < XAE_DMA_MASK_MIN ||
      dma_mask > XAE_DMA_MASK_MAX) {
    dev_info(&pdev->dev, "DMA missing/invalid xlnx,addrwidth property ret=%d (%d), using default.\n",
        ret, dma_mask);
    lp->dma_mask = XAE_DMA_MASK_MIN;
  }
  else {
    lp->dma_mask = (u8) (dma_mask & 0xFF);
    dev_info(&pdev->dev, "DMA addrwidth=%d.\n",
        (u32) lp->dma_mask);
  }
					    ...
					    struct net_device *ndev)
{
	...
	for_each_rx_dma_queue(lp, i) {
		...
		if (np) {
			...
			q->eth_hasdre = of_property_read_bool(np, "xlnx,include-dre");
      dev_info(&pdev->dev, "DMA using DRE=%s\n",
        q->eth_hasdre ? "true" : "false");
...
static int axienet_probe(struct platform_device *pdev)
{
	...
	ret = of_get_mac_address_address(pdev->dev.of_node, mac_addr);
	if (!ret) {
	...
	ret = of_get_phy_mode_mode(pdev->dev.of_node, &lp->phy_mode);
	if (ret < 0)
